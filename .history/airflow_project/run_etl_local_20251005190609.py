...
class LocalETLRunner:
    ...
    def decide_cleaning_strategy(self, quality_reports):
        ...
        for source_name, report in quality_reports.items():
            ...
            if score < 80:
                needs_cleaning = True
                print(f"   {source_name}: Score {score}% - Nettoyage requis")
            else:
                print(f"   {source_name}: Score {score}% - Qualité acceptable")
        
        if needs_cleaning:
            self.log_step("Stratégie", "Nettoyage nécessaire - Amélioration activée")
            return 'improve'
        else:
            self.log_step("Stratégie", "Qualité suffisante - Intégration directe")
            return 'integrate'
    ...
    def improve_data_quality(self, datasets):
        ...
        for source_name, df in datasets.items():
            ...
            actions = sum(improvement_summary['by_category'].values())
            
            self.log_step(f"Amélioration {source_name}", 
                         f"{actions} actions d'amélioration appliquées")
